 ...
 * @version 1.0 */
final class DriverPool4x4 implements Driver {

	private static final Map<String, PoolConnectionInfo> CONNECTIONS_BY_KEY = new ConcurrentHashMap<>(64, 0.1f);

	private static final String URL_PREFFIX = "jdbc:pool(4,4):";

	private static final int URL_PREFFIX_LENGTH = DriverPool4x4.URL_PREFFIX.length();

	static {
		try {
			DriverManager.registerDriver(new DriverPool4x4());
		} catch (final SQLException e) {
			throw new RuntimeException(e);
		}
	}

	private DriverPool4x4() {
	...
	public final boolean acceptsURL(final String url) {
		...
		return url.startsWith(DriverPool4x4.URL_PREFFIX);
	}

	@Override
	public final Connection connect(final String url, final Properties info) throws SQLException {

		if (!url.startsWith(DriverPool4x4.URL_PREFFIX)) {
			return null;
		}
		final String key1 = PoolConnectionInfo.getKey1(url, info);
		PoolConnectionInfo ci = DriverPool4x4.CONNECTIONS_BY_KEY.get(key1);
		if (ci == null) {
			final String key2 = PoolConnectionInfo.getKey2(url, info);
			ci = DriverPool4x4.CONNECTIONS_BY_KEY.get(key2);
			if (ci == null) {
				final boolean created;
				synchronized (this) {
					ci = DriverPool4x4.CONNECTIONS_BY_KEY.get(key2);
					if (ci == null) {
						ci = new PoolConnectionInfo(4, 4, DriverPool4x4.URL_PREFFIX_LENGTH, url, info);
						DriverPool4x4.CONNECTIONS_BY_KEY.put(key1, ci);
						DriverPool4x4.CONNECTIONS_BY_KEY.put(key2, ci);
				...
				if (created) {
					new PoolFillerThread(this, info, url, 4 * 4).start();
				}
			}
		}
	...
	public final DriverPropertyInfo[] getPropertyInfo(final String url, final Properties info) throws SQLException {
		...
		final String realUrl = url.substring(DriverPool4x4.URL_PREFFIX_LENGTH);
	...
	public String toString() {
		...
		return DriverPool4x4.URL_PREFFIX;
	}
}
